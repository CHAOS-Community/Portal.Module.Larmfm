namespace Chaos.Portal.Module.Larmfm.View
{
  ...
  public class SearchView : AView
  {
    ...
    private static readonly Guid ScheduleNoteMetadataSchemaGuid =
      new UUID("70c26faf-b1ee-41e8-b916-a5a16b25ca69").ToGuid();
  ...
  public class SearchViewData : IViewData
  {
    public IEnumerable<KeyValuePair<string, string>> GetIndexableFields()
    {
      ...
      if (!string.IsNullOrEmpty(FreeText))
        yield return new KeyValuePair<string, string>("FreeText", FreeText + " " + FreeTextAnnotation);
      ...
      if (!string.IsNullOrEmpty(AnnotationCount))
        yield return new KeyValuePair<string, string>("AnnotationCount", AnnotationCount);

      if (!string.IsNullOrEmpty(AttachedFilesCount))
        yield return new KeyValuePair<string, string>("AttachedFilesCount", AttachedFilesCount);

      if (!string.IsNullOrEmpty(DomsId))
        yield return new KeyValuePair<string, string>("DomsId", DomsId);
      ...
      if (ObjectFolders != null)
        ...
        {
          yield return
            new KeyValuePair<string, string>("FolderID", objectFolder.ID.ToString(CultureInfo.InvariantCulture));
        }
    }

    public KeyValuePair<string, string> UniqueIdentifier
    {
      get { return new KeyValuePair<string, string>("Id", Identifier); }
    }

    public string Fullname
    {
      get { return "Chaos.Portal.Module.Larm.Data.SearchViewData"; }
    }
