namespace Chaos.Portal.Module.Larmfm.View
{
  ...
  public class LarmObjectViewData : ObjectViewData
  {
    ...
    public override IEnumerable<KeyValuePair<string, string>> GetIndexableFields()
    {
      ...
      var programMetadata =
        Object.Metadatas.FirstOrDefault(
          item => item.MetadataSchemaGuid == Guid.Parse("00000000-0000-0000-0000-0000df820000"));

      if (programMetadata != null)
      {
        var root = programMetadata.MetadataXml.Root;

        if (root != null && root.Element("PublicationDateTime") != null &&
            root.Element("PublicationEndDateTime") != null)
        {
          ...
          yield return
            new KeyValuePair<string, string>("LARM-PubStartDate",
                                             larmPubStartDate.ToString("yyyy'-'MM'-'dd'T'HH':'mm':'ss'Z'",
                                                                       formatProvider));
          yield return
          ...
          if (root.Element("Title") != null)
            yield return
              new KeyValuePair<string, string>("LARM-Title",
                                               root.Element("Title").Value.Trim().TrimStart('"').TrimEnd('"'));
        }
      }
      ...
      yield return
        new KeyValuePair<string, string>("LARM-Annotation-Count",
                                         ObjectRelationInfos.Count(
                                           item => item.Object2TypeID == 41 || item.Object2TypeID == 64).ToString());
